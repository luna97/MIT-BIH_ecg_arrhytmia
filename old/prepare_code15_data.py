...
from dataset.helper_code_code15 import is_integer, is_boolean, sanitize_boolean_value
...
def run(args):
    ...
    print('Loading patient demographic data...')

    with open(args.data_folder + 'exams.csv', 'r') as f:
        for i, l in enumerate(f):
    ...
    print(f'Loaded {len(exam_id_to_patient_id)} exam IDs.')
    ...
    # get all the files in the data folder ending with hdf5
    signal_files = [os.path.join(args.data_folder, f) for f in os.listdir(args.data_folder) if f.endswith('.hdf5')]
    ...
    def process_signal_file(signal_file):
        with h5py.File(signal_file, 'r') as f:
            ...
            print(f'Processing {num_exam_ids} exam IDs in {signal_file}...')
            ...
            for i in range(num_exam_ids):
                ...
                comments = [f'Exam ID: {exam_id}', f'Patient ID: {patient_id}', f'Age: {age}', f'Sex: {sex}']
                ...
                # every 1000 records, print the progress
                if i % 1000 == 0:
                    print(f'Processed {i} records for {signal_file}')

    Parallel(n_jobs=-1)(delayed(process_signal_file)(signal_file) for signal_file in signal_files)
