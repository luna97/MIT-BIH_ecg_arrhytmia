...
def train(config, run=None, wandb=False):
    ...
    if config.use_class_weights:
        ...
        weights = torch.tensor([2.2248e-01, 1.0810e+01, 2.6938e+00, 2.4588e+01, 1.2755e+03]).to('cuda')
    else:
        weights = None
    ...
    if wandb:
        ...
        trainer = L.Trainer(max_epochs=config.epochs, logger=wand_logger, callbacks=[checkpoint_callback, early_stopping, lr_monitor])
    else:
        trainer = L.Trainer(max_epochs=config.epochs, callbacks=[checkpoint_callback, early_stopping, lr_monitor])
